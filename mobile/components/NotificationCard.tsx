...
interface NotificationCardProps {
  ...
  onDelete: (token: string | null, notificationId: string) => void;
}
...
const NotificationCard = ({ notification, onDelete }: NotificationCardProps) => {
  ...
  const getNotificationText = () => {
    const name = `${notification.from.username} `;
  ...
  const handleDelete = () => {
    Alert.alert("Delete Notification", "Are you sure you want to delete this notification?", [
      { text: "Cancel", style: "cancel" },
      {
        text: "Delete",
        style: "destructive",
        onPress: () => onDelete(token,notification._id),
      },
    ]);
  };
    ...
    <View className="border-b border-gray-100 bg-white">
      <View className="flex-row p-4">
        <View className="relative mr-3 ">
          <Image
            source={{ uri: notification.from.profilePic }}
